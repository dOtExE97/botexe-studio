...
- Registriert wird ein Widget in **`apps/desktop/src/renderer/pages/widget-types.ts`** (`WIDGET_TYPES`): `type`, `label`, `desc` (in Erzähler-Form: was es dem Streamer bringt), `w/h`, Standard-`props`, `fields`. Kategorie in `CATEGORY_OF`; verwandte Widgets in `RELATED_OF` (klappen in der Palette unter einem Anführer zusammen).
