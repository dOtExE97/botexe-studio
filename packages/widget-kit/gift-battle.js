...
.bx-bt-title { font-family: var(--bx-font-display); font-size: calc((clamp(12px,min(3cqi,8.5cqh),38px)) * var(--bx-fs, 1)); letter-spacing:.16em;
  text-transform:uppercase; color: var(--bx-text,#fff); text-shadow: 0 0 14px color-mix(in srgb, var(--bx-accent) 55%, transparent); }
.bx-bt-clock { font-family: var(--bx-font-mono); font-weight:800; font-size: calc((clamp(12px,min(2.8cqi,8cqh),36px)) * var(--bx-fs, 1)); color: var(--bx-gold);
...
.bx-bt-name { font-family: var(--bx-font-display); font-size: calc((clamp(11px,min(2.8cqi,8cqh),38px)) * var(--bx-fs, 1)); -webkit-text-stroke:2px #0a0b12;
...
.bx-bt-score { font-family: var(--bx-font-mono); font-weight:800; font-size: calc((clamp(12px,min(3.4cqi,10cqh),46px)) * var(--bx-fs, 1)); color:#fff;
...
.bx-bt-vs { align-self:center; font-family: var(--bx-font-display); font-size: calc((clamp(15px,min(4.2cqi,12cqh),54px)) * var(--bx-fs, 1)); color:#fff;
...
.bx-bt-banner { position:absolute; left:50%; top:50%; transform:translate(-50%,-50%) scale(.6); z-index:5; opacity:0;
  font-family: var(--bx-font-display); font-size: calc((clamp(15px,min(5.4cqi,15cqh),64px)) * var(--bx-fs, 1)); color: var(--bx-gold); -webkit-text-stroke:3px #0a0b12;
...
/* ── „Rahmen ausblenden" (.bx-frameless): ohne Panel steht der Text direkt auf
   ...
   darum Kontur an Titel, Uhr und Punktestand (die Namen haben schon eine).
